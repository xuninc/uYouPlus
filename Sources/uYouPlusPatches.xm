...
- (void)updateRelatedVideos {
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"relatedVideosAtTheEndOfYTVideos"]) %orig;
}
%end
...
%ctor {
    ...
    if (![allKeys containsObject:@"automaticallyCheckForUpdates"])
        [defaults setBool:NO forKey:@"automaticallyCheckForUpdates"];

    // Disable uYou's welcome screen (fix #1147)
    if (![allKeys containsObject:@"showedWelcomeVC"])
        [defaults setBool:YES forKey:@"showedWelcomeVC"];

    // Disable uYou's disable age restriction
    if (![allKeys containsObject:@"disableAgeRestriction"])
        [defaults setBool:NO forKey:@"disableAgeRestriction"];

    // Disable uYou's adblock
    if (![allKeys containsObject:@"removeYouTubeAds"])
